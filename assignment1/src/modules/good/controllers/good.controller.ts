...
export class GoodController {
  ...
  async filterGoods(@Body() filters: FilterGoodsDto) {
    const goods = await this.goodService.filterGoodsViaDetails(filters);
    return {goods};
  }

  @Get('translated-details')
  async getTranslatedGood(@Body('_id') _id: string) {
    const good = await this.goodService.getTranslatedGoodById(_id);
    return good;
  }

  @Get('stats')
  async get() {
    const stats = await this.goodService.countGoodsByCategory();
    return {stats};
  }
  ...
  async getOriginalGood(@Body('_id') _id: string) {
    const good = await this.goodService.getOriginalGoodById(_id);
    return good;
  }
  ...
  async addGood(@Body() addGoodDto: AddGoodDto) {
    const good = await this.goodService.addGood(addGoodDto);
    return {good};
  }
  ...
  async editGood(@Body() editGoodDto: EditGoodDto) {
    const good = await this.goodService.editGood(editGoodDto);
    return {good};
  }
  ...
  async removeGood(@Body('_id') _id: string) {
    const good = await this.goodService.deleteGoodById(_id);
    return {good};
  }
}
